...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        x = self.fc1(x)#Linear hidden_features这是一个中间特征维度，用于在 MLP 模块中执行非线性变换
        x = self.act(x)#gelu
        x = self.drop(x)#dropout
        x = self.fc2(x)#Linear-这才是图中的Linear
        x = self.drop(x)#dropout
        return x
        ...
        **kwargs,
        ...
        if False:
            ...
            if forward_type.startswith("xv1"):
                self.in_proj = nn.Conv2d(d_model, d_inner + dt_rank + 8 * d_state, 1, bias=bias)

            if forward_type.startswith("xv2"):
                self.in_proj = nn.Conv2d(d_model, d_inner + d_inner + 8 * d_state, 1, bias=bias)
            ...
            if forward_type.startswith("xv3"):
                ...
                self.in_proj = nn.Conv2d(d_model, d_inner + 4 * dt_rank + 8 * d_state, 1, bias=bias)
            ...
            if forward_type.startswith("xv61"):
                self.forward = partial(self.forwardxv, mode="xv1")
                self.in_proj = Linear2d(d_model, d_inner + dt_rank + 8 * d_state, bias=bias)
                self.out_act = nn.GELU()
        ...
        if True:
            ...
            if forward_type.startswith("xv2a"):
                ...
                self.in_proj = Linear2d(d_model, d_inner + d_inner + 8 * d_state,bias=bias)

            if forward_type.startswith("xv3a"):
                self.forward = partial(self.forwardxv, mode="xv3a", omul=omul)
                self.in_proj = Linear2d(d_model, d_inner + 4 * dt_rank + 8 * d_state,bias=bias)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        '''self.cnn_branch = nn.Sequential(
            nn.Conv2d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            nn.Conv2d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=3, padding=1, bias=True)
        )
        self.norm_cnn = norm_layer(hidden_dim)'''
    def _forward(self, input: torch.Tensor):
        ...
        # Normalize input before CNN branch
        '''normed_input = self.norm_cnn(input)  # Apply normalization before passing to CNN branch
        cnn_features = self.cnn_branch(normed_input.permute(0, 3, 1, 2))  # (B, H, W, C) -> (B, C, H, W)
        cnn_features = cnn_features.permute(0, 2, 3, 1)  # (B, C, H, W) -> (B, H, W, C)
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            downsample = _make_downsample(
                self.dims[i_layer], #包含每一层的通道数
                self.dims[i_layer + 1], 
                norm_layer=norm_layer,
                channel_first=self.channel_first,
            ) if (i_layer < self.num_layers - 1) else nn.Identity()

            self.layers.append(self._make_layer(#用于创建个层
                dim = self.dims[i_layer],#当前层的通道数
