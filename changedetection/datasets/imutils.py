...
def random_fliplr2(pre_img, mid_img, post_img, label):
    if random.random() > 0.5:
        pre_img = np.fliplr(pre_img)
        mid_img = np.fliplr(mid_img)
...
def con_random_fliplr(pre_img, post_img, lama_img,label):
    if random.random() > 0.5:
        label = np.fliplr(label)
        pre_img = np.fliplr(pre_img)
        post_img = np.fliplr(post_img)
        lama_img= np.fliplr(lama_img)

    return pre_img, post_img, lama_img,label
def single_random_fliplr(pre_img ,label):
    if random.random() > 0.5:
        label = np.fliplr(label)
        pre_img = np.fliplr(pre_img)
...
def random_fliplr_build(img, label_1):
    if random.random() > 0.5:
        label_1 = np.fliplr(label_1)
        img = np.fliplr(img)
...
def random_fliplr_mcd(pre_img, post_img, label_cd,  label_2):
    if random.random() > 0.5:
    ...
    return pre_img, post_img, label_cd, label_2
def random_fliplr_mcd(pre_img, post_img, label_cd,  label_2):
    if random.random() > 0.5:
        label_cd = np.fliplr(label_cd)
        label_2 = np.fliplr(label_2)
...
def random_flipud2(pre_img, mid_img, post_img, label):
    if random.random() > 0.5:
        pre_img = np.flipud(pre_img)
        mid_img = np.flipud(mid_img)
...
def random_flipud(pre_img, post_img, label):
    ...
    return pre_img, post_img, label
def con_random_flipud(pre_img, post_img, lama_img,label):
    if random.random() > 0.5:
        label = np.flipud(label)
        pre_img = np.flipud(pre_img)
        post_img = np.flipud(post_img)
    ...
    return pre_img, post_img, lama_img,label
def single_random_flipud(pre_img,label):
    if random.random() > 0.5:
        label = np.flipud(label)
        pre_img = np.flipud(pre_img)
...
def random_flipud_bda(pre_img, post_img, label_1, label_2):
    ...
    return pre_img, post_img, label_1, label_2
def random_flipud_build(img, label_1):
    if random.random() > 0.5:
        label_1 = np.flipud(label_1)
        img = np.flipud(img)
...
def random_flipud_mcd(pre_img, post_img, label_cd, label_2):
    if random.random() > 0.5:
...
def random_rot2(pre_img, mid_img, post_img, label):
    k = random.randrange(3) + 1  # 随机选择旋转 90/180/270 度
    pre_img = np.rot90(pre_img, k).copy()
    mid_img = np.rot90(mid_img, k).copy()
...
def random_rot(pre_img, post_img, label):
    ...
    return pre_img, post_img, label
def con_random_rot(pre_img, post_img, lama_img,label):
    k = random.randrange(3) + 1

    pre_img = np.rot90(pre_img, k).copy()
    post_img = np.rot90(post_img, k).copy()
    label = np.rot90(label, k).copy()
    lama_img = np.rot90(lama_img, k).copy()

    return pre_img, post_img, lama_img,label

def single_random_rot(pre_img,  label):
    k = random.randrange(3) + 1
...
def random_rot_build(img, label_1):
    k = random.randrange(3) + 1
...
def random_rot_mcd(pre_img, post_img, label_cd, label_2):
    k = random.randrange(3) + 1
...
def random_crop_new2(pre_img, mid_img,post_img, label, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    h, w = label.shape
    ...
    pad_mid_image = np.zeros((H, W, 3), dtype=np.float32)
    pad_post_image = np.zeros((H, W, 3), dtype=np.float32)
    pad_label = np.ones((H, W), dtype=np.float32) * ignore_index

    # 填充均值 RGB
    for c in range(3):
        pad_pre_image[:, :, c] = mean_rgb[c]
        pad_mid_image[:, :, c] = mean_rgb[c]
        pad_post_image[:, :, c] = mean_rgb[c]
    ...
    pad_mid_image[H_pad:(H_pad + h), W_pad:(W_pad + w), :] = mid_img
    ...
    # 执行裁剪
    pre_crop = pad_pre_image[H_start:H_end, W_start:W_end, :]
    mid_crop = pad_mid_image[H_start:H_end, W_start:W_end, :]
    post_crop = pad_post_image[H_start:H_end, W_start:W_end, :]
    label_crop = pad_label[H_start:H_end, W_start:W_end]

    return pre_crop, mid_crop, post_crop, label_crop
...
def con_random_crop_new(pre_img, post_img, lama_img,label, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    h, w = label.shape
    ...
    pad_post_image = np.zeros((H, W, 3), dtype=np.float32)
    pad_lama_image = np.zeros((H, W, 3), dtype=np.float32)
    pad_label = np.ones((H, W), dtype=np.float32) * ignore_index
    ...
    pad_pre_image[H_pad:(H_pad + h), W_pad:(W_pad + w), :] = pre_img
    pad_post_image[H_pad:(H_pad + h), W_pad:(W_pad + w), :] = post_img
    pad_lama_image[H_pad:(H_pad + h), W_pad:(W_pad + w)] = lama_img
    pad_label[H_pad:(H_pad + h), W_pad:(W_pad + w)] = label
...
def single_random_crop_new(pre_img,  label, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    h, w = label.shape
...
def random_crop_build(img, loc_label, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    h, w = loc_label.shape
    ...
    H_start, H_end, W_start, W_end = get_random_cropbox()
    # print(W_start)
    pre_img = pad_image[H_start:H_end, W_start:W_end, :]
    loc_label = pad_loc_label[H_start:H_end, W_start:W_end]

    return pre_img,  loc_label


def random_crop_mcd(pre_img, post_img, label_cd, label_2, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    h, w = label_2.shape
...
def random_crop_mcd_1(pre_img, post_img, label_cd,label_1, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    h, w = label_1.shape
    ...
    return pre_img, post_img, label_cd, label_1